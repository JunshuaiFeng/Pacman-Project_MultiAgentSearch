...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        dist = [-manhattanDistance(f, newPos) for f in foodList]
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def value(state, agentIndex, depth):
            ...
            nextValue = []
            for a in actions:
                successor = value(state.generateSuccessor(agentIndex, a), nextAgent, d)
    ...
    def getAction(self, gameState):
        ...
        def value(state, agentIndex, depth, alpha, beta):
            ...
            if agentIndex == 0:  # max state
                v = float('-inf')
                for a in actions:
                    successor = value(state.generateSuccessor(agentIndex, a), nextAgent, d, alpha, beta)[0]
                    if successor >= v:
                        v = successor
                        localAction = a
                    alpha = max(alpha, v)
            ...
            return [v, localAction]

        bestAction = value(gameState, self.index, self.depth, float('-inf'), float('inf'))[1]
        return bestAction
